...
public class XTCTrajectoryParserCSharp {
    ...
    public static int GetFrameCount(string filename) {
        ...
        try {
              ...
              /*  if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.WindowsEditor)
                {
                    ReposudoeManager jr = GameObject.FindFirstObjectByType<ReposudoeManager>();
                    if (jr.binaryData.Length > 0)
                        reader = new BinaryReader(new MemoryStream(jr.binaryData));
                }
                else
                {*/
    ...
    /// <summary>
    /// Devuelve el número de átomos en el primer frame del archivo de trayectoria.
    /// </summary>
    /// <param name="filename"></param>
    /// <returns></returns>
    public static int GetAtomCount(string filename)
    {
        try
        {
            using (BinaryReader reader = CreateBinaryReader(filename))
            {
        ...
        catch (Exception ex)
        {
            ...
            throw new Exception($"Error en GetAtomCount: {ex.Message}", ex);
        }
    }

    /// <summary>
    /// Crea un BinaryReader adecuado según la plataforma.
    /// </summary>
    private static BinaryReader CreateBinaryReader(string filename)
    {
        /*if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.WindowsEditor)
        {
            ReposudoeManager jr = GameObject.FindFirstObjectByType<ReposudoeManager>();
            if (jr != null && jr.binaryData != null && jr.binaryData.Length > 0)
                return new BinaryReader(new MemoryStream(jr.binaryData));
            else
                throw new FileNotFoundException("No se encontró ReposudoeManager o binaryData es nulo/vacío.");
        }
        else
        {*/
